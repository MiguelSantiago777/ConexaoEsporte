...
def test_exportar_termo_responsabilidade_reflete_dados_do_polo(client, seed_basico):
    ...
    # representante, sem problema). O endereço usado no termo é o do
    # próprio polo/instituição, não um endereço pessoal separado.
    client.patch(
    ...
    assert "End A" in texto_completo  # endereço do próprio polo (seed_basico)
